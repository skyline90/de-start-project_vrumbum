...
CREATE TABLE IF NOT EXISTS car_shop.customers (
    ...
    person_name VARCHAR, -- Тип данных VARCHAR выбран, так как имена могут иметь разную длину, и тип VARCHAR позволяет хранить строки любой длины без необходимости указывать максимальный размер
    phone VARCHAR -- Тип данных VARCHAR выбран, так как номера могут содержать не только цифры, но и символы (например, +, -, x для добавочных номеров)
);
...
CREATE TABLE IF NOT EXISTS car_shop.cars (
    ...
    auto VARCHAR NOT NULL, -- выбран VARCHAR позволяет не ограничивать длину строки. Ограничение NOT NULL означает, что поле обязательно должно быть заполнено, так как автомобиль без названия не имеет смысла.
    color VARCHAR, -- выбрано поле color использует тип VARCHAR, так как цвет автомобиля может быть любым текстовым значением, включая названия цветов или их комбинации 
    ...
    brand_origin VARCHAR -- выюроан Тип данных VARCHAR используется для хранения страны происхождения бренда, так как это строковое значение, и страны могут быть разной длины
);
